...
bool MyChild::loadFile(const QString &fileName)//导入文件
{
    ...
    //提供一个字节数组,QByteArray可用于存储原始字节(包括"\0")和传统的8位"\0"端字符串
    QByteArray data =file.readAll();

    //Qt使用Unicode 来存储.绘制,操作字符串,在许多的情况下,可能希望处理不同的编码的数据
    QTextCodec *codec =Qt::codecForHtml(data);
    QString str =codec->toUnicode(data);

    if (Qt::mightBeRichText(str)){//如果是富文本就设置为HTml
        this->setHtml(str);
    }else{//如果不是富文本设置为简单的字符串格式
...
void MyChild::documentWasModifiled()
{
    //在设置改变时,设置窗口已修改
...
QString MyChild::userFriendlyCurrentFile()//用户友好当前文件
{
...
void MyChild::setAlign(int align) //段落对齐设置
{
    if(align == 1)
        ...
        this->setAlignment(Qt::AlignRight | Qt::AlignAbsolute);//水平方向靠右,绝对
    else if (align ==4)
...
void MyChild::setStyle(int style)
{
    ...
    if(style !=0){
        ...
        case 6:
            ...
            break;
        case 7:
            stylename = QTextListFormat::ListUpperAlpha;//大写罗马数字,最大4999
            break;
        case 8:
        ...
        //QTextEdit类就是一个富文本编辑器,所以在构造QTextEdit类的对象时就已经构建了一个QTextDocument类对象和
        //一个QTextCursor类对象,只须调用他们进行相应的操作即可
...
//    cursor.beginEditBlock();   // 开始一个操作块（可以一次撤销）

//    QTextBlockFormat blockFmt = cursor.blockFormat(); // 获取当前段落格式
//    QTextListFormat listFmt;

//    if (cursor.currentList()) { // 如果当前段落已经在列表里
//        listFmt = cursor.currentList()->format(); // 获取现有列表格式
//    } else {                     // 如果没有列表
//        listFmt.setIndent(blockFmt.indent() + 1); // 列表缩进 = 当前段落缩进 + 1
//        blockFmt.setIndent(0);                    // 段落缩进重置为 0
//        cursor.setBlockFormat(blockFmt);          // 应用修改后的段落格式
//    }

//    listFmt.setStyle(stylename); // 设置列表样式（如项目符号、编号等）
//    cursor.createList(listFmt);  // 创建或更新列表
//    cursor.endEditBlock();       // 结束操作块

//    } else { // 如果不是创建列表
//        QTextBlockFormat bfmt;
//        bfmt.setObjectIndex(-1);     // 取消段落中的对象（如列表）
//        cursor.mergeBlockFormat(bfmt); // 应用到当前段落
//    }
}
